...
class NormalizeDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        if self.normalize_coord:
            ...
            #     print(index, self.coordinates, coordinates.shape)
...
class NormalizeWithRefDataset(BaseWrapperDataset):
    def __init__(self, dataset, coordinates, ref_coordinates, normalize_coord=True):
        self.dataset = dataset
        self.coordinates = coordinates
    ...
    def __cached_item__(self, index: int, epoch: int):
        dd = self.dataset[index].copy()
        coordinates = dd[self.coordinates]
        ref_coordinates = dd[self.ref_coordinates]
        # normalize
        if self.normalize_coord:
            coordinates = coordinates - ref_coordinates.mean(axis=0)
            dd[self.coordinates] = coordinates.astype(np.float32)
        return dd
