...
class CrossMultiheadAttention(nn.Module):
    def __init__(
        ...
        # print("self.head_dim: ", self.head_dim)    # 8      
        # sys.exit()
        self.scaling = (self.head_dim * scaling_factor) ** -0.5
    ...
    def forward(
        ...
        # print("q.shape: ", q.shape)      # torch.Size([512, 24, 8])
        # print("k.shape: ", k.shape)      # torch.Size([512, 56, 8])
        # print("v.shape: ", v.shape)      # torch.Size([512, 56, 8])
        # sys.exit()
        ...
        # Softmax + dropout (with optional bias)
        if not return_attn:
            attn = softmax_dropout(
                attn_weights, self.dropout, self.training, bias=attn_bias
            )
        else:
            attn_weights = attn_weights + (attn_bias if attn_bias is not None else 0)
            attn = softmax_dropout(
                attn_weights, self.dropout, self.training, inplace=False
            )
