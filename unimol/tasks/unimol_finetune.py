...
class UniMolFinetuneTask(UnicoreTask):
    ...
    def load_dataset(self, split, **kwargs):
        ...
        if split == "train":
            ...
            sample_dataset = ConformerSampleDataset(
                dataset, self.args.seed, "atoms", "coordinates", "func_groups", "fg_atom"
            )
            dataset = AtomTypeDataset(dataset, sample_dataset)
        else:
            dataset = TTADataset(
                dataset, self.args.seed, "atoms", "coordinates", "func_groups", "fg_atom", self.args.conf_size
            )
    ...
    def load_dataset2(self, split, **kwargs):
        ...
        dataset = AddFunctionGroupDataset(
            dataset, "smi", "atoms", "coordinates", "func_groups", "fg_atom"
        )  
        # print("dataset[0]: ", dataset[0])
