    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            if predicts.size(-1) == 1:
                ...
                metrics.log_scalar(f"{split}_mae", mae, sample_size, round=5)
                metrics.log_scalar(f"{split}_mse", mse, sample_size, round=5)
                metrics.log_scalar(f"{split}_rmse", np.sqrt(mse), sample_size, round=5)
                metrics.log_scalar(f"{split}_agg_mae", agg_mae, sample_size, round=5)
                metrics.log_scalar(f"{split}_agg_mse", agg_mse, sample_size, round=5)
                metrics.log_scalar(f"{split}_agg_rmse", np.sqrt(agg_mse), sample_size, round=5)
...
class FinetuneSmoothMAELoss(FinetuneMSELoss):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        # print("loss in compute_loss: ", loss)
        return loss
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        if "valid" in split or "test" in split:
            ...
            # print(np.abs(y_pred - y_true).mean(axis=0,keepdims=True).shape)
            # print(np.abs(y_pred - y_true).mean(axis=1,keepdims=True).shape)
            # sys.exit()
            agg_mae = np.abs(y_pred - y_true).mean(axis=0,keepdims=True).mean(axis=1)
            # print("agg_mae: ", agg_mae)
            metrics.log_scalar(f"{split}_agg_mae", agg_mae, sample_size, round=5)
